...
// Map project names to cover images
...
function getBase() {
  // Prefer explicit public base if set
  if (process.env.NEXT_PUBLIC_BASE_URL) return process.env.NEXT_PUBLIC_BASE_URL;
  // Vercel-style env
  if (process.env.VERCEL_URL) return `https://${process.env.VERCEL_URL}`;
  // Render sometimes exposes this
...
async function getData() {
  ...
  const [siteRes, skillsRes, projectsRes, servicesRes] = await Promise.all([
    fetch(`${base}/api/site`, { cache: "no-store" }),
    fetch(`${base}/api/skills`, { cache: "no-store" }),
